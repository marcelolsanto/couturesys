...
class FinanceiroAdvancedTests(TestCase):
    ...
    def test_regressao_nao_perder_historico_pago(self):
        ...
        não deve sumir do caixa (ou deve ser tratada com cuidado).
        Nota: O comportamento padrão do Django é CASCADE (apaga tudo).
        Este teste serve para te alertar sobre isso.
        """
        ...
        # Cria a conta financeira vinculada e marca como PAGA (dinheiro entrou)
        ...
        # AÇÃO DE RISCO: Deletar o pedido original
        with self.assertRaises(ProtectedError):
            pedido.delete()

        # Opcional: Garantir que a conta ainda existe
        self.assertTrue(ContaReceber.objects.filter(id=conta.id).exists())

        # VERIFICAÇÃO: O dinheiro sumiu do caixa?
        # Se este teste falhar (conta.exists() == False), significa que deletar pedidos
        # altera seu histórico financeiro passado. Isso é perigoso para contabilidade.
        conta_existe = ContaReceber.objects.filter(id=conta.id).exists()



        #if not conta_existe:
            #print("\n⚠️ ALERTA DE REGRESSÃO: Deletar um pedido apagou o registro financeiro de uma conta já paga!")
            # Em sistemas reais, usamos on_delete=models.PROTECT ou models.SET_NULL para evitar isso.
